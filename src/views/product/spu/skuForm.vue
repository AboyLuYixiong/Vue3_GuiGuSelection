<template>
  <el-form label-width="100px">
    ...
    <el-form-item label="重量(克)" v-model="skuParams.weight">
      <el-input placeholder="重量(克)" type="number"></el-input>
    </el-form-item>
    <el-form-item label="SKU描述" v-model="skuParams.skuDesc">
    ...
    <el-form-item label="图片名称">
      <el-table border :data="imgArr" ref="table">
        ...
        <el-table-column type="图片">
          <template #="{ row, $index }">
            <img :src="row.logoUrl" style="width:100px;height:100px;">
          </template>
        </el-table-column>
        <el-table-column type="名称"></el-table-column>
...
const save = async () => {
  ...
  skuParams.skuSaleAttrValueList = saleArr.value.reduce((prev: any, next: any) => {
    if (next.attrIdAndValueId) {
      let [saleAttrId, saleValueId] = next.saleIdAndValueId.split(':')
      prev.push({
        saleAttrId,
        saleValueId
      })
